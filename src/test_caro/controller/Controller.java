 ...
 * 
 * Controller.java, 04/10/2017 Luu Thanh Sang
 */
...
public class Controller {
	...
	int row = CommonConfig.CHESS_BOARD_ROW; // Hoành độ tương ứng với bàn cờ
	int column = CommonConfig.CHESS_BOARD_COLUMN; // Tung độ tương ứng với bàn cờ
	 ...
	 */
	public void getComputerTurn(ArrayList<ChessPosition> chessPositions) {
		boolean match = false; // Biến đánh dấu so khớp thế cờ
		for (ChessPosition chessPosition : chessPositions) { // Duyệt tất cả các thế cờ
			Position position = findChessPosition(chessPosition); // So khớp từng thế cờ với các ma trận trên bàn cờ
		...
		if (!match) { // Nếu không so khớp thế cờ nào
			if (checkWin(randomPosition())) { // G�?i hàm đánh ngẫu nhiên và kiểm tra thắng thua
				ChessBoard.showMessage("You lose"); // Nếu máy thắng, show pop-up
				resetChessBoard(); // Reset lại bàn cờ
			}
		}
	 ...
	 */
	public Position findChessPosition(ChessPosition chessPosition) {
	 ...
	 * @return position : vị trí tọa độ ô cờ cần đánh
	 */
	public Position matchChessPosition(int row, int column, ChessPosition chessPosition) {
		Position position = null; // Khởi tạo ô cờ cần đánh
		String[][] chessPositionMatrix = chessPosition.getChessPosition();
		...
		for (int i = 0; i < 5; i++) {
			for (int j = 0; j < 5; j++) {
				...
				if ("X".equals(chessPositionMatrix[i][j]) && !"X".equals(squareVal)) { // Xét trường hợp ô cờ chứa quân X
					return null; // Trả về null
				}
				if ("O".equals(chessPositionMatrix[i][j]) && !"O".equals(squareVal)) { // Xét trường hợp ô cờ chứa quân O
					return null; // Trả về null
				}
				if ("T".equals(chessPositionMatrix[i][j]) && !"".equals(squareVal)) { // Xét trường hợp ô cờ trống
					return null; // Trả về null
				}
				// Thiết lập tọa độ ô cờ cần đánh nếu so khớp với giá trị "D"
				if ("D".equals(chessPositionMatrix[i][j]) && "".equals(squareVal)) { // So khớp với giá trị "D"
		...
		for (int i = -4; i <= 4; i++) { // Lấy ô cờ vừa đánh làm trung tâm, duyệt 8 ô cờ trên dưới theo chiều dọc
			if (x + i >= 0 && x + i < row) { // Các ô cờ được duyệt 2 bên phải có hoành độ >= 0 và < số hàng của bàn cờ
				...
				if (squares[x + i][y].getText().equals(value)) { // Nếu những ô cờ được xét trùng giá trị với ô cờ vừa đánh
					...
					count++; // Tăng biến đếm
					if (count == 5) { // Nếu có 5 quân cùng loại li�?n mạch nhau
						return true; // Trả về true
					}
